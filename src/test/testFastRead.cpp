...
#include <libmaus/fastx/SubSamplingFastReader.hpp>
...
{
	if ( ::libmaus::fastx::IsFastQ::isFastQ(filename) )
	{
		readFastXTemplate< ::libmaus::fastx::FastQReaderSplit > (filename);
	}
	else
	{
		readFastXTemplate< ::libmaus::fastx::FastAReaderSplit > (filename);
	}
}

#include <libmaus/fastx/SlashStrip.hpp>
#include <libmaus/fastx/FirstWhiteSpaceSlashStrip.hpp>

int main(int argc, char * argv[])
{
	::libmaus::fastx::FirstWhiteSpaceSlashStrip SlS;
	...
	std::vector < ::libmaus::fastx::FastInterval > VV = ::libmaus::fastx::FastQReader::computeCommonNameAlignedFrags(std::vector<std::string>(1,fn),16,2,SlS);
	::libmaus::fastx::FastQReader FQO(fn);
	::libmaus::fastx::FastQReader::pattern_type opattern;
	...
	for ( uint64_t i = 0; i < VV.size(); ++i )
	{
		...
		::libmaus::fastx::FastQReader FQI(fn,VV[i]);
		::libmaus::fastx::FastQReader::pattern_type ipattern;
	...
	try
	{
		#if 0
		std::cerr << "Indexing...";
		std::vector< ::libmaus::fastx::FastInterval> ints = ::libmaus::fastx::FastQReader::parallelIndex(
