...
#include <libmaus/wavelet/DynamicHuffmanWaveletTree.hpp>
...
void reorderHuffmanTreeZero(::libmaus::huffman::HuffmanTreeNode * hnode)
{
	...
	std::map < int, ::libmaus::huffman::HuffmanTreeLeaf * > hleafmap;
...
void applyRankMap(::libmaus::huffman::HuffmanTreeNode * hnode, std::map<int,uint64_t> const & rankmap)
{
	std::map < int, ::libmaus::huffman::HuffmanTreeLeaf * > hleafmap;
	hnode->fillLeafMap(hleafmap);

	for ( std::map < int, ::libmaus::huffman::HuffmanTreeLeaf * >::iterator ita = hleafmap.begin(); ita != hleafmap.end(); ++ita )
	{
...
void computeBWT(std::string const & textfilename, std::ostream & output)
{
	...
	::libmaus::wavelet::DynamicHuffmanWaveletTree<k,w> B(ahnode);
	::libmaus::cumfreq::DynamicCumulativeFrequencies scf(257);
