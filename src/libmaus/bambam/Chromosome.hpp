	...
	namespace bambam
	{
		...
		struct ChromosomeVectorMerge
		{
			...
			struct ChromosomeIndexComparator : public libmaus::bambam::StrCmpNum
			{
				...
				bool const comprareNonSNLN;
				
				ChromosomeIndexComparator(std::vector < std::vector<Chromosome> const * > const & rV, bool const rcomprareNonSNLN = false)
				: V(rV), comprareNonSNLN(rcomprareNonSNLN) {}
			...
			ChromosomeVectorMerge(std::vector < std::vector<Chromosome> const * > const & V, bool const comprareNonSNLN = false)
			: chromosomes(), chromosomesmap(), topological(false)
			{
				ChromosomeIndexComparator comp(V,comprareNonSNLN);
