	...
	namespace bitio
	{
		struct CompactDecoderBuffer : public ::std::streambuf
		{
			...
			: stream(filename),
			  ...
			  // smallest multiple aligning to 64 bits
			  alignmult(1ull << (6-::libmaus::bitio::Ctz::ctz(b))),
			...
			int_type underflow()
			{
				...
				uint64_t const wordstoread = (symstoread * b + 63)/64;
				uint64_t const bytestoread = wordstoread * 8;
