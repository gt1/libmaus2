...
namespace libmaus
{
	...
	{
		/**
		 * two level rank dictionary using approximately n/2 bits for index.
		 ...
		 **/
		struct ERank22 : public ERankBase
		{
			public:
			...
			{
				if ( n & mbmask )
					throw ::std::runtime_error("libmaus::rank::ERank22: n is not multiple of miniblock size 32.");
			...
			unsigned int rank1(unsigned int i) const
			{
				...
				return S[i >> sbbitwidth] + M[mi] + popcount4( reorder4(UUUU[mi]), i - (mi<<mbbitwidth));
			}
			/**
