		...
		struct BandedLocalEditDistance : public LocalEditDistanceTraceContainer
		{
			...
			)
			{
				...
				if ( k )
				{
					...
					for ( int64_t pskip = 1; (b - bb) != m; ++pskip )
					{
						assert ( ((p-M.begin()) % k21) == 0 );
						assert ( ((q-M.begin()) % k21) == 0 );
					...
					uint64_t const offfront = p-M.begin();
					uint64_t const offfrontb = (offfront / k21);
					...
					return LocalEditDistanceResult(numins,numdel,nummat,nummis,offfronta,offfrontb,n-offbacka,m-offbackb);
				}
				else
				{
					assert ( k == 0 );
					...
					return LocalEditDistanceResult(0,0,nummat,nummis,0,0,0,0);
				}
			}
		};
	}
