...
**/
#if ! defined(LIBMAUS_FASTX_COMPACTFASTQMULTIBLOCKREADER_HPP)
#define LIBMAUS_FASTX_COMPACTFASTQMULTIBLOCKREADER_HPP

#include <libmaus/fastx/CompactFastQSingleBlockReader.hpp>
...
namespace libmaus
{
	namespace fastx
	{
		struct CompactFastQMultiBlockReaderBase
		{
			...
			{
				return ::libmaus::fastx::CompactFastDecoder::loadIndex(filename);
			}
			
			static uint64_t getNonEmptyDataSize(std::string const & filename)
			{
				std::vector< ::libmaus::fastx::FastInterval > const index = loadIndex(filename);
				if ( ! index.size() )
					return 0;
				else
			...
			static ::std::vector < ::libmaus::aio::FileFragment > getFragments(std::vector<std::string> const & filenames)
			{
				...
				for ( uint64_t i = 0; i < filenames.size(); ++i )
				{
					...
					frags.push_back(::libmaus::aio::FileFragment(fn,offset,length));
			...
			static ::std::vector < ::libmaus::aio::FileFragment > getFragments(std::string const & filename)
			{
		...
		struct CompactFastQMultiBlockReader
		{
			...
			typedef ::libmaus::fastx::CompactFastQSingleBlockReader<stream_type> single_block_reader_type;
		...
		struct MultiFileCompactFastQDecoder
		{
			...
			::libmaus::aio::ReorderConcatGenericInput<uint8_t> stream;
			CompactFastQMultiBlockReader< ::libmaus::aio::ReorderConcatGenericInput<uint8_t> > reader;
			typedef CompactFastQMultiBlockReader< ::libmaus::aio::ReorderConcatGenericInput<uint8_t> >::pattern_type pattern_type;
