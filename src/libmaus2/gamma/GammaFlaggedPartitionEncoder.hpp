...
namespace libmaus2
{
	namespace gamma
	{
		struct GammaFlaggedPartitionEncoder
		{
			...
			libmaus2::autoarray::AutoArray< FlaggedPartitionInterval > B;
			FlaggedPartitionInterval * const pa;
			FlaggedPartitionInterval * pc;
			FlaggedPartitionInterval * const pe;
			...
			void flushInternal()
			{
				...
				if ( numintv )
				{
					...
					for ( FlaggedPartitionInterval * pp = pa; pp != pc; ++pp )
						assert( pp->to > pp->from );
					// check intervals are touching
					for ( FlaggedPartitionInterval * pp = pa+1; pp != pc; ++pp )
					...
					FlaggedPartitionInterval * pp = pa;
					while ( pp != pc )
					{
						FlaggedPartitionInterval * pup = pp;
						while ( pup != pc && pup->complete == pp->complete )
							++pup;

						G.encodeWord(pp->complete,1);
			...
			void put(FlaggedPartitionInterval const & P)
			{
				*(pc++) = P;
				if ( pc == pe )
					flushInternal();
