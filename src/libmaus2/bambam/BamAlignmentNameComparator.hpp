...
namespace libmaus2
{
	namespace bambam
	{
		 ...
		 **/
		struct BamAlignmentNameComparator : public StrCmpNum, public DecoderBase
		{
			//! data pointer
			uint8_t const * data;
		...
		struct BamAlignmentNameObject
		{
			...
			{
				return DecoderBase::getLEInteger(D + P[i],4);
			}
		};
		...
		struct BamAlignmentNameProjector
		{
			...
			{
				return BamAlignmentNameComparator::compare(A[i],A.length(i),A[j],A.length(j));
			}
		};
	}
}
