...
*/
#if ! defined(LIBMAUS2_DAZZLER_ALIGN_APPROXIMATERUNPARTCONTAINER_HPP)
#define LIBMAUS2_DAZZLER_ALIGN_APPROXIMATERUNPARTCONTAINER_HPP
...
namespace libmaus2
{
	namespace dazzler
	{
		namespace align
		{
			...
			{
				ApproximateRunPartContainer(libmaus2::dazzler::db::DatabaseFile const & DB)
				: PartTrackContainer(DB,"app_runs") 
				{
				}

				std::vector<libmaus2::dazzler::align::ApproximateRun> getApproximateRuns(uint64_t const readid) const
				{
					...
					uint64_t roffset = 0;
					uint64_t const numel = libmaus2::dazzler::db::InputBase::getLittleEndianInteger8(appistr,roffset);
