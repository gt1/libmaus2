	...
	namespace suffixsort
	{
		namespace bwtb3m
		{
			struct MergeStrategyMergeInternalSmallBlock : public MergeStrategyMergeBlock
			{
				...
				MergeStrategyMergeInternalSmallBlock(std::istream & in) : MergeStrategyMergeBlock(in) {}

				bool equal(MergeStrategyBlock const & O) const
				{
					if ( dynamic_cast<MergeStrategyMergeInternalSmallBlock const *>(&O) == 0 )
						return false;
				...
				{
					libmaus2::util::NumberSerialisation::serialiseNumber(out,MergeStrategyBlock::merge_block_type_internal_small);
