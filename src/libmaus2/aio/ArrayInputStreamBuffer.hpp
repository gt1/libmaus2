...
*/
#if !defined(LIBMAUS2_AIO_ARRAYINPUTSTREAMBUFFER_HPP)
#define LIBMAUS2_AIO_ARRAYINPUTSTREAMBUFFER_HPP
...
namespace libmaus2
{
	namespace aio
	{
		...
		struct ArrayInputStreamBuffer : public ::std::streambuf
		{
			public:
			typedef _iterator iterator;
			typedef ArrayInputStreamBuffer<iterator> this_type;
			...
			ArrayInputStreamBuffer(ArrayInputStreamBuffer const &);
			ArrayInputStreamBuffer & operator=(ArrayInputStreamBuffer &);
			...
			inline void setgchecked(char * a, char * b, char * c)
			{
				...
				if ( ! ok )
				{
					...
					lme.getStream() << "ArrayInputStreamBuffer: invalid parameters for setg detected, a="
			...
			void init(bool const repos)
			{
				...
				if ( repos )
				{
					uint64_t const newpos = std::min(symsread,filesize);
					itc = ita + newpos;
				}
			}

			ssize_t memread(char * p, uint64_t const s)
			{
				uint64_t const maxcopy = ite-itc;
				uint64_t const tocopy = std::min(maxcopy,s);
				std::copy(itc,itc+tocopy,p);
				itc += tocopy;
				return tocopy;
			}

			public:
			ArrayInputStreamBuffer(iterator rita, iterator rite, int64_t const rblocksize, uint64_t const rputbackspace = 0)
			:
			  ita(rita), ite(rite), itc(ita),
			  filesize(ite-ita),
			...
			int_type underflow()
			{
				...
				// load data
				uint64_t const uncompressedsize = memread(
