...
*/
#if ! defined(LIBMAUS2_HUFFMAN_LFPHIPAIRLCPDECODER_HPP)
#define LIBMAUS2_HUFFMAN_LFPHIPAIRLCPDECODER_HPP
...
#include <libmaus2/huffman/LFPhiPairLCP.hpp>
#include <libmaus2/util/iterator.hpp>
...
namespace libmaus2
{
	namespace huffman
	{
		struct LFPhiPairLCPDecoder : public libmaus2::gamma::GammaPDIndexDecoderBase
		{
			typedef LFPhiPairLCPDecoder this_type;
			typedef libmaus2::util::unique_ptr<this_type>::type unique_ptr_type;
			typedef libmaus2::util::shared_ptr<this_type>::type shared_ptr_type;

			typedef LFPhiPairLCP value_type;
			...
			libmaus2::autoarray::AutoArray<LFPhiPairLCP> B;
			...
			struct R1Projector { static void set(LFPhiPairLCP & P, uint64_t const v) { P.r1 = v; } };
			struct P1Projector { static void set(LFPhiPairLCP & P, uint64_t const v) { P.p1 = v; } };
			struct LCPProjector { static void set(LFPhiPairLCP & P, uint64_t const v) { P.lcp = v; } };
			...
			bool decodeBlock()
			{
				...
				uint64_t const numobj = decodeSequence<R1Projector>();
				decodeSequence<P1Projector>();
				decodeSequence<LCPProjector>();
			...
			LFPhiPairLCPDecoder(std::vector<std::string> const & rVfn, uint64_t const offset)
			: index(rVfn), FBO(), PISI(), PSGI(), B(), pa(0), pc(0), pe(0), rlbuffer()
			{
				init(offset);
			}

			bool decode(LFPhiPairLCP & v)
			{
				while ( pc == pe )
				{
