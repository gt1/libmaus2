...
*/
#if ! defined(LIBMAUS2_HUFFMAN_LFSYMRANKPOSDECODER_HPP)
#define LIBMAUS2_HUFFMAN_LFSYMRANKPOSDECODER_HPP
...
#include <libmaus2/huffman/LFSymRankPos.hpp>
...
namespace libmaus2
{
	namespace huffman
	{
		struct LFSymRankPosDecoder : public libmaus2::gamma::GammaPDIndexDecoderBase
		{
			typedef LFSymRankPosDecoder this_type;
			typedef libmaus2::util::unique_ptr<this_type>::type unique_ptr_type;
			typedef libmaus2::util::shared_ptr<this_type>::type shared_ptr_type;

			typedef LFSymRankPos value_type;
			...
			libmaus2::autoarray::AutoArray<LFSymRankPos> B;
			...
			LFSymRankPosDecoder(std::vector<std::string> const & rVfn, uint64_t const offset)
			: index(rVfn), FBO(), PISI(), PSGI(), B(), pa(0), pc(0), pe(0), V(), rlbuffer()
			{
				init(offset);
			}

			bool decode(LFSymRankPos & v)
			{
				while ( pc == pe )
				{
			...
			static uint64_t getLength(std::vector<std::string> const & Vfn, uint64_t const
				#if defined(_OPENMP)
				numthreads
				#endif
			)
